...
\titexit{Populus} spp.  (cottonwoods) and arthropods,
...
species' impact another with zero being no effect and values less than
or greater than zero as negative and positive effects,
respectively. Here, we will refer to this matrix ($\mathbf{D}$) as a
signed, weighted interaction matrix. It therefore has the properties
that it can be asymmetric (i.e. $D_{ij}$ does not necessarily equal
$D_{ji}$) and it does not scale between 0 and 1, . The diagonal
($D_{ii}$) is zero, and does not have the mathematical properties of a
probabilistic network. Also, as the method does not track individuals
