...
\item Genotype influenced lichen network structure
  ...
  \item Networks were dominated by positive links (mean +- SE), and
    in general we observed far fewer negative links among lichen
    species (mean +- SE).
  \item Only positive links responded to tree genotype (see h2-net.tex)
...
\item Genotype impacts were on positive interactions mainly driven by Ch
  \begin{itemize}
  \item Centrality varied among species (ANOVA)
  \item \textit{Caloplaca holocarpa} centrality was the main species
    to exhibit a significant response to tree genotype in terms of
    overall centrality (\textit{RLRT} = ?, $H^2$ = ?, \textit{p-value}
    = ?), as well as positive centrality for both in- (\textit{RLRT} =
    ?, $H^2$ = ?, \textit{p-value} = ?) and out-degree (\textit{RLRT}
    = ?, $H^2$ = ?, \textit{p-value} = ?).
  \item Although \textit{X. montana} positive out-degree centrality
    also varied significantly among tree genotypes (\textit{RLRT} = ?,
    $H^2$ = ?, \textit{p-value} = ?), this response was small ($H^2$ =
    ?) and driven by two genotypes 
...
\caption{Bivariate plots of the relationship between bark roughness
  and three network metrics: A) number of links (L), B) centralization
  and C) average mutual information (AMI). Each plot displays the
  genotype mean $\pm$ 1 S.E. for both variables and a least-squares
  regression calculated using the genotype means.}
\label{fig:br_net}
\end{figure*}
...
\input{muse.tex}
\input{h2_reml.tex}
\input{h2_reml_net.tex}
\input{vec.tex}
\input{h2_reml_trait.tex}
