...
constructed interaction networks for the lichens and conducted genetic
analyses of several network metrics. We found three primary
...
Thrid, although multiple species were observed repeatedly in the
interaction networks, the effect of tree genotype was primarily
focused on variation of the interactions of one lichen species,
\textit{Caloplaca holocarpa}. Third, one of the examined tree traits,
bark roughness, was both predicted by tree genotype and correlated
with lichen network similarity, supporting a mechanistic pathway from
a variration in a heritable tree trait and the genetically based
variation in lichen network structure. We conclude that tree genotype
can influence not only the relative abundances of organisms but also
the interaction network structure of associated organisms. Given that
