...
%% Currently: 257 words 06November2019
...
number of network links and network centrality were both correlated
with bark roughness; however, none of the network metrics examined
were significantly correlated with network similarity and this
variation is unlikely what is underpinning the genetically based
similarity of lichen networks. We conclude that tree genotype
influences lichen interaction network structure with two potential
pathways through bark roughness and condensed tannin
concentration. These results provide empirical evidence that variation
in ecological interaction networks can result from genetically based
variation in foundation species in ecosystems.
...
%% Integrate this figure into the methods
%% Use it to show the output of the network modeling.
...
% fig:cn_onc
%% Figure now remade with CT added and NA's removed
%
% 1. Genotype predicted network similarity 2. Several tree traits,
% including bark roughness, condensed tannins and bark CN ratio, but
% not pH, predicted network similarity 3. Lichen network similarity
% was correlated with network metrics, number of links, centrality and
% modularity, but, although correlated with several tree traits (bark
% roughness and condensed tannings) none of these was predicted by
% tree genetics

Network similarity was genetically based. Tree genotype was a
significant predictor of network similarity (R$^2$ = 0.23, F$_{10}$ =
19.30, \textit{p-value} $\leq$ 0.001). The abundance (R$^2$ = 0.001,
F$_{1}$ = 3.19, \textit{p-value} $\leq$ 0.08), richness (R$^2$ =
0.001, F$_{1}$ = 3.19, \textit{p-value} $\leq$ 0.08) and evenness
(R$^2$ = 0.001, F$_{1}$ = 3.19, \textit{p-value} $\leq$ 0.08) of the
bark lichen communities along with the lichen network structural
metrics that we examined (number of links, modularity and centrality)
were all correlated with lichen network similarity (Table
~\ref{tab:cn_perm}). Interestingly, lichen network similarity was not
correlated with lichen community composition either when species
abundances were relativized (Mantel R = 0.09, \textit{p-value} =
0.139) or not (Mantel R = -0.03, \textit{p-value} = 0.573). Also,
unlike lichen network similarity , lichen community composition was
not predicted by tree genotype (Supplementary
Tables~\ref{tab:com_perm}).

Several tree traits and lichen community and network metrics were
significant predictors of lichen network similarity. Tree bark
roughness (R$^2$ = 0.01, F$_{1}$ = 10.29, \textit{p-value} $\leq$
0.001), carbon to nitrogen ratio (R$^2$ = 0.03, F$_{1}$ = 25.10,
\textit{p-value} $\leq$ 0.001), and condensed tannins (R$^2$ = 0.05,
F$_{1}$ = 44.78, \textit{p-value} $\leq$ 0.001) were significantly
correlated with lichen network similarity; however, bark pH (R$^2$ =
0.001, F$_{1}$ = 3.19, \textit{p-value} $\leq$ 0.08) was not (Table
~\ref{tab:cn_perm}).
...
% C. subdeflexa was the most central species, primarily positive but
% tended to have a negative relationship with X. galariculata

% Mean C. subdeflexa centrality = 0.73 +/- 0.14 (with 1 being
% perfectly central a star)
%
...
% 1.
% 2. 
% 3. Lichen composition genetically based
% 4. None of the structural networks were genetically based
...
%% Are patterns similar to wild stands
%% Wild stands don't have chemistry data
